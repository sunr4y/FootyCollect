...
class TestE2EItemCreationTests(TestCase):
    ...
    def test_create_item_without_api_integration(self):
        ...
        # Pattern: CLUB_NAME + SEASON + " - " + SIZE + " - " + [Fan|Player] Version
        # Since is_fan_version=False and no kit.type (no FKAPI), expected:
        expected_name = f"{self.club.name} {self.season.year} - {self.size.name} - Player Version"
        item = BaseItem.objects.get(name=expected_name, user=self.user)
        self.assertRedirects(
            response,
            reverse("collection:item_detail", kwargs={"pk": item.pk}),
        )
    ...
    def test_create_item_with_api_integration(self, mock_get_kit_details):
        ...
        # Name is auto-generated by build_name() when Jersey is saved
        # Pattern: CLUB_NAME + TYPE_K + SEASON + " - " + SIZE + " - " + [Fan|Player] Version
        # With FKAPI data, kit has type="Home", and is_fan_version=False, so expected:
        expected_name = f"Real Madrid Castilla Home 2023-24 - {self.size.name} - Player Version"
        item = BaseItem.objects.get(name=expected_name, user=self.user)
    ...
    def test_create_item_verifies_all_attributes(self):
        ...
        # Name is auto-generated by build_name() when Jersey is saved
        # Format: club season - size - player#number - version - long sleeve - signed
        # Since is_fan_version=True, player_name="Messi", number=10,
        # is_short_sleeve=False, is_signed=True, and no kit.type (no FKAPI):
        expected_name = (
            f"{self.club.name} {self.season.year} - {self.size.name} - Messi#10 - Fan Version Long Sleeve - Signed"
        )
        item = BaseItem.objects.get(name=expected_name, user=self.user)
    ...
    def test_create_item_page_shows_no_errors(self):
        ...
        # Name is auto-generated by build_name() when Jersey is saved
        # Get the latest item created for this user
        item = BaseItem.objects.filter(user=self.user).order_by("-id").first()
        self.assertIsNotNone(item, "Item should be created")

        # Verify the name was auto-generated (not the form name)
        self.assertNotEqual(item.name, "Error Test Jersey")
        # Verify the name contains expected components
        self.assertIn(self.club.name, item.name)
        ...
        # The template shows get_display_name_with_type() which returns "CLUB_NAME + TYPE_K"
        # So we should check for the display name, not the full item.name
        if item.jersey:
            display_name = item.jersey.get_display_name_with_type()
            # The display name should be in the title
            self.assertIn(display_name, content, f"Display name '{display_name}' not found in content")
            # The season should be shown separately
            if item.season:
    ...
    def test_create_item_with_photos(self):
        ...
        # Verify the name was auto-generated (not the form name)
        self.assertNotEqual(item.name, "Jersey With Photos")
        # Verify the name contains expected components
        self.assertIn(self.club.name, item.name)
        self.assertIn(str(self.season.year), item.name)
        self.assertIn(self.size.name, item.name)
    ...
    def test_create_item_with_api_failure_graceful_degradation(self, mock_get_kit_details):
        ...
        # Verify the name was auto-generated (not the form name)
        self.assertNotEqual(item.name, "Jersey API Failure Test")
        # Verify the name contains expected components
        self.assertIn(self.club.name, item.name)
        self.assertIn(str(self.season.year), item.name)
        self.assertIn(self.size.name, item.name)
    ...
    def test_item_detail_page_renders_all_attributes(self):
        ...
        # Create Kit with type for this test
        ...
        Jersey.objects.create(base_item=item, size=self.size, is_fan_version=True, number=10, kit=kit)
        # Name is auto-generated by build_name() when Jersey is saved
        # Refresh to get the generated name
        ...
        self.assertEqual(response.context["item"], item)
        # The template shows get_display_name_with_type() which returns "CLUB_NAME + TYPE_K"
        # So it should show "Real Madrid Castilla Home" (club + type) in the title
        # and the season "2023-24" separately below
        self.assertIn("Real Madrid Castilla Home", content)
    ...
    def test_create_item_with_competitions(self):
        ...
        # Verify the name was auto-generated (not the form name)
        self.assertNotEqual(item.name, "Jersey With Competitions")
        # Verify the name contains expected components
        self.assertIn(self.club.name, item.name)
        self.assertIn(str(self.season.year), item.name)
        self.assertIn(self.size.name, item.name)
    ...
    def test_create_item_with_full_api_data(self, mock_get_kit_details):
        ...
        # Verify the name was auto-generated (not the form name)
        self.assertNotEqual(item.name, "Real Madrid Castilla 2023-24 Home")
        # Verify the name contains expected components
        self.assertIn("Real Madrid Castilla", item.name)
        self.assertIn("2023-24", item.name)
        self.assertIn(self.size.name, item.name)
        # With FKAPI data, kit should have type="Home"
        self.assertIn("Home", item.name)
    ...
    def test_complete_flow_real_madrid_castilla_kit(self, mock_get_kit_details):
        ...
        for required in required_content:
            self.assertIn(required, content, f"Required content '{required}' not found")

        content_lower = content.lower()
        error_indicators = ["traceback", "django exception", "server error", "internal server error"]
        for indicator in error_indicators:
            self.assertNotIn(indicator, content_lower, f"Error indicator '{indicator}' found in content")
