...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        brand_names = [
            ...
            'Reebok', 'Umbro', 'Kappa', 'Diadora', 'Joma',
            'Macron', 'Hummel', 'Uhlsport', 'Errea', 'Mizuno'
        ]
        
        # Extender la lista para tener más de 50 nombres únicos
        extended_names = brand_names + [
            f"{fake.company()} Sports" for _ in range(35)
        ]

        for name in extended_names:
            Brand.objects.create(
                id_fka=random.randint(1000, 9999),
                name=name,
                slug=slugify(name),
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully created {len(extended_names)} Brand items')
        ) 